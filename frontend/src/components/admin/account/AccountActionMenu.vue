      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button @click="$emit('view-mapping', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="swap" size="sm" class="text-cyan-500" />
              {{ t('admin.accounts.viewModelMapping') }}
            </button>
...
const emit = defineEmits(['close', 'test', 'stats', 'view-mapping', 'reauth', 'refresh-token', 'reset-status', 'clear-rate-limit'])
const { t } = useI18n()
